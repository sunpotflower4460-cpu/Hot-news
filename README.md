...
Node.js 24 LTSを使用します。Node.js 20はサポート終了済みのため使用しません。
...
- モック記事とモックAPIの無効化
- サポート中のNode.js LTSと安全なNext.js最低バージョン
...
1. 本番DB、公開読取API、編集管理認証を作る
2. 手動で記事を登録・公開・隔離・訂正・撤回できるようにする
3. クライアントを実行時API取得へ変更し、静的モック依存を除く
4. 候補収集、重複排除、出典確認、AI補助を追加
5. 通知を掲載ゲートと撤回機能へ接続
6. Capacitor/Xcodeプロジェクト、Privacy Manifest、署名、実機試験
7. 必要性が確認できた後にStoreKit課金やアカウント同期を追加
